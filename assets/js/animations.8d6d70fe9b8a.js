...
(function () {
  ...
  /**
   * The mast's navy field follows native scroll on every viewport, with pointer
   * depth on fine-pointer desktop only. Outer groups own pointer depth; inner
   * groups own scroll depth, keeping text and link hit areas still.
   */
  function addHomeMastField(signal, portable) {
    ...
    var layers = [
      {
        el: back,
        depth: 0.55,
        scrollY: portable ? -28 : 34,
      },
      {
        el: front,
        depth: 1,
        scrollY: portable ? -44 : 56,
      },
    ];
    layers.forEach(function (layer, index) {
      layer.scrollTo = gsap.quickTo(drifts[index], "y", {
    ...
    function updateScrollDepth() {
      if (!visible) return;
      // The compact SVG is scaled down independently from desktop. Keep its
      // depth visible and bounded in screen pixels, including wide touch screens.
      var matrix = portable ? drifts[0].ownerSVGElement.getScreenCTM() : null;
      var scale = matrix ? Math.max(0.01, Math.hypot(matrix.c, matrix.d)) : 1;
      layers.forEach(function (layer) {
        layer.scrollTo(scrollProgress * layer.scrollY / scale);
      });
    }
