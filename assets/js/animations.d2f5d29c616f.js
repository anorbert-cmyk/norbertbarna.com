...
(function () {
  ...
  function addWorkListHover(signal) {
    ...
    surface.className = "work-list-surface";
    surface.setAttribute("aria-hidden", "true");
    highlight.className = "work-list-highlight";
    surface.appendChild(highlight);
    list.prepend(surface);
    list.setAttribute("data-work-hover", "");

    // Created inside the existing matchMedia context: reuse these four
    // animations on input, then let that context revert them on teardown.
    gsap.set(highlight, { x: 0, y: 0, scaleY: 1, transformOrigin: "0 0" });
    var xTo = gsap.quickTo(highlight, "x", { duration: 0.3, ease: "power3.out" });
    var yTo = gsap.quickTo(highlight, "y", { duration: 0.35, ease: "power3.out" });
    var heightTo = gsap.quickTo(highlight, "scaleY", { duration: 0.35, ease: "power3.out" });
    var visibility = gsap.timeline({ paused: true }).fromTo(surface, { opacity: 0 }, {
    ...
    function place(row) {
      ...
      highlight.style.backgroundColor = getComputedStyle(row).getPropertyValue("--work-accent").trim();
      yTo(row.offsetTop);
      heightTo(row.offsetHeight);
      if (visibility.progress() === 0) {
        yTo.tween.progress(1);
        heightTo.tween.progress(1);
      }
    }
    ...
    rows.forEach(function (row) {
      ...
      row.addEventListener("pointermove", function (event) {
        ...
        // No layout reads or new tweens in the high-frequency handler.
        xTo(Math.max(-6, Math.min(6, ((event.clientX - bounds.left) / bounds.width - 0.5) * 12)));
      }, { signal: signal });
      row.addEventListener("pointerleave", function (event) {
        if (event.pointerType === "touch" || activeRow !== row) return;
        activeRow = null;
        xTo(0);
        visibility.reverse();
      }, { signal: signal });
    ...
    // Keyboard feedback is immediate CSS, never a moving focus target.
    ...
    return function () {
      ...
      bounds = null;
      surface.remove();
      list.removeAttribute("data-work-hover");
    };
  }

  /**
   * The mast's navy field follows the pointer and native scroll. Outer groups
   * own pointer depth; inner groups own scroll depth, keeping both responses
   * independent and every text/link hit area still. No perpetual idle loop.
   */
  function addHomeMastField(signal) {
    ...
    var layers = [
      {
        ...
        scrollTo: gsap.quickTo(drifts[0], "y", { duration: 0.68, ease: "power2.out" }),
      },
      {
        el: front,
        depth: 1,
        ...
        scrollTo: gsap.quickTo(drifts[1], "y", { duration: 0.68, ease: "power2.out" }),
      },
    ];
    var visible = false;
    ...
    function resetPointerDepth() {
      layers.forEach(function (layer) {
        ...
        layer.yTo(0);
        if (!visible) {
          layer.xTo.tween.progress(1).pause();
          layer.yTo.tween.progress(1).pause();
        }
      });
    }

    function handlePointer(event) {
      if (!visible || reducedMotion || event.pointerType === "touch") return;
      if (!bounds) bounds = mast.getBoundingClientRect();
      if (!bounds.width || !bounds.height) return;
      var nx = Math.max(-1, Math.min(1, ((event.clientX - bounds.left) / bounds.width - 0.5) * 2));
      var ny = Math.max(-1, Math.min(1, ((event.clientY - bounds.top) / bounds.height - 0.5) * 2));
      // Keep diagonal movement within the same 24px radius as horizontal input.
      var distance = Math.max(1, Math.hypot(nx, ny));
      var x = nx * 24 / distance;
      var y = ny * 18 / distance;
      layers.forEach(function (layer) {
    ...
    function updateScrollDepth() {
      if (!visible) return;
      layers.forEach(function (layer) {
        layer.scrollTo(scrollProgress * layer.scrollY);
      });
    }
    ...
    function updateVisibility() {
      visible = intersecting && !document.hidden && !reducedMotion && !signal.aborted;
      bounds = null;
      if (visible) updateScrollDepth();
      else {
        resetPointerDepth();
        layers.forEach(function (layer) {
          layer.scrollTo.tween.pause();
        });
      }
    }

    document.addEventListener("visibilitychange", updateVisibility, { signal: signal });
    window.addEventListener("resize", function () { bounds = null; }, { signal: signal });
