/** Stable home/compact navigation with a native-scroll case utility journey. */
(function () {
  'use strict';
  ...
  if (footer && isCase) footer.after(landing);
  var frame = 0;
  var suspended = false;
  var lastCount = '';
  var keyboard = false;
  ...
  function render() {
    ...
    // Home has one continuous top bar. Its hero chapter still morphs, but never
    // hands navigation back to the case-only travelling/reading-slot behavior.
    var stationary = isHome || compact.matches || motionOff || focused || menuOpen;
    var travel = stationary ? 0 : Math.max(0, innerHeight - header.offsetHeight) * ratio;
    var safe = isHome ? { y: 0, dock: false, slot: 'home' } :
      compact.matches ? { y: 0, dock: false, slot: 'compact' } :
