...
(function () {
  ...
  function awardVisible(state) {
    // These decorative backgrounds are hidden on compact devices and only
    // revealed by the existing desktop hover treatment. Never decode them all.
    return !state.award || (hoverQuery.matches && state.award.matches(":hover"));
  }
  ...
  function measure(state) {
    var r = state.video.getBoundingClientRect();
    var style = window.getComputedStyle(state.video);
  ...
  var nearObserver = hasObserver ? new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      var state = stateByVideo.get(entry.target);
      if (!state) return;
  ...
  function init() {
    document.querySelectorAll("video[data-autoplay-video]").forEach(function (video) {
      var state = {
        video: video, award: video.closest(".awards-card"), visible: false,
      ...
      if (state.award) {
        ...
        state.award.addEventListener("pointerleave", function () { stop(state); });
      }
      measure(state);
      reconcile(state);
      if (nearObserver) nearObserver.observe(video);
      if (visibleObserver) visibleObserver.observe(video);
    });
    document.querySelectorAll("[data-motion-toggle]").forEach(function (button) {
