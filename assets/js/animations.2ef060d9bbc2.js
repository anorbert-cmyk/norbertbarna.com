...
(function () {
  ...
  /**
   * Desktop groups split pointer and scroll depth. Portable motion translates
   * the already-painted root SVG as one composited CSS layer: changing its
   * filtered inner groups every frame would repeatedly rasterize the artwork.
   */
  function addHomeMastField(signal, portable) {
    var mast = document.querySelector(".home-mast");
    if (!mast) return function () {};
    var art = mast.querySelector(".home-mast-art");
    ...
    if (!art || !back || !front || drifts.length !== 2) return function () {};

    var layers = portable ? [{ el: art, scrollY: -44 }] : [
      {
        el: back,
        depth: 0.55,
        scrollY: 34,
    ...
    if (portable) {
      mast.setAttribute("data-mast-motion", "portable");
      gsap.set(art, { y: 0, force3D: true });
    }
    layers.forEach(function (layer, index) {
      var scrollOptions = {
        duration: portable ? 0.48 : 0.68, ease: "power2.out",
      };
      if (portable) scrollOptions.force3D = true;
      layer.scrollTo = gsap.quickTo(portable ? art : drifts[index], "y", scrollOptions);
      if (!portable) {
        layer.xTo = gsap.quickTo(layer.el, "x", { duration: index ? 0.72 : 0.82, ease: "power3.out" });
        layer.yTo = gsap.quickTo(layer.el, "y", { duration: index ? 0.72 : 0.82, ease: "power3.out" });
      }
    });
    var visible = false;
    ...
    function updateScrollDepth() {
      if (!visible) return;
      layers.forEach(function (layer) {
        // Root CSS translation is already in screen pixels; desktop inner
        // groups retain their existing SVG coordinate distances.
    ...
    return function () {
      ...
      scrollTrigger.kill();
      layers.forEach(function (layer) {
        if (layer.xTo) layer.xTo.tween.kill();
        if (layer.yTo) layer.yTo.tween.kill();
        layer.scrollTo.tween.kill();
      });
      if (portable) {
        gsap.set(art, { clearProps: "transform" });
        mast.removeAttribute("data-mast-motion");
      } else {
  ...
  function initDesktopMotion() {
    ...
    if (isHome) {
      removeHomeMastField = addHomeMastField(listeners.signal, false);
  ...
  function initPortableMotion() {
    ...
    if (isHome) {
      removeHomeMastField = addHomeMastField(listeners.signal, true);
