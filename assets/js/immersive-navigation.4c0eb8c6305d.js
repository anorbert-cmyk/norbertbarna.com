/** Native-scroll utility header, mirroring KODE's top-to-bottom journey. */
(function () {
  'use strict';
  ...
  function render() {
    ...
    var travel = motionOff || focused || menuOpen ? 0 : Math.max(0, innerHeight - header.offsetHeight) * ratio;
    var safe = composition ? { y: 0, dock: false, slot: 'composition' } : readingClearance(travel, motionOff || focused || menuOpen);
    travel = heldTravel !== null && !motionOff && !focused && !menuOpen ? heldTravel : safe.y;
    var changingSlot = previousTravel !== null && previousSlot !== safe.slot && Math.abs(travel - previousTravel) > header.offsetHeight * 2;
    if (composition || motionOff || keyboard || focused || menuOpen || heldTravel !== null) cancelSlotFade();
    ...
    var brandScale = motionOff || focused || menuOpen ? 1 : 1 + terminal * (maxScale - 1);
    var brandOffset = 0;
    if (terminal > 0 && !motionOff && !focused && !menuOpen) {
    ...
    header.style.setProperty('--nav-end', String(motionOff || focused || menuOpen ? 0 : terminal));
    header.toggleAttribute('data-footer-zone', Boolean(footerVisible && !motionOff && !focused && !menuOpen));
