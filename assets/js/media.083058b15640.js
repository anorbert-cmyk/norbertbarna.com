...
(function () {
  ...
  function shouldPlay(state) {
    return !isReduced() && !suspended && !document.hidden && state.visible &&
      !state.failed && !state.previewEnded &&
      (state.previewDeadline === null || performance.now() < state.previewDeadline) &&
      awardVisible(state);
  }

  function beginHeaderPreview(state) {
    if (!state.headerPreview || state.previewDeadline !== null) return;
    // One wall-clock window from the first real playback. Pausing, scrolling
    // and page lifecycle changes must not give decorative headers a new loop.
    state.previewDeadline = performance.now() + 4500;
  ...
  function init() {
    document.querySelectorAll("video[data-autoplay-video]").forEach(function (video) {
      var state = {
        ...
        headerPreview: Boolean(video.closest(".case-study-header")),
        previewDeadline: null, previewEnded: false
      };
      states.push(state);
