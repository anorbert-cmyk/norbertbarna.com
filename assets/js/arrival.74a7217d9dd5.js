...
(function () {
  ...
  if ((!isHome && !isCase) || reduced.matches || root.classList.contains("no-motion") ||
  ...
  try {
    if (sessionStorage.getItem("nb-arrival-seen-v2")) return;
  ...
  window.addEventListener("pointerdown", function (event) {
    if (!enter.contains(event.target)) finish("pointer");
