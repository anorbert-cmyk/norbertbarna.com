...
(function () {
  ...
  /** Native view timelines own supported browsers; GSAP is the fallback. */
  function addHomeMastField() {
    var mast = document.querySelector(".home-mast");
    var art = mast && mast.querySelector(".home-mast-art");
    if (!art) return function () {};
    var nativeTimeline = window.CSS && CSS.supports("animation-timeline: --home-fold-scroll") &&
      CSS.supports("animation-range: exit-crossing 0% exit-crossing 100%");
    if (nativeTimeline) return function () {};
    var tween = gsap.to(art, {
      yPercent: -13, xPercent: 3, rotationX: 6, rotationY: -16, rotation: 9,
      ease: "none",
      scrollTrigger: {
        trigger: mast, start: "top top", end: "bottom top", scrub: .45,
